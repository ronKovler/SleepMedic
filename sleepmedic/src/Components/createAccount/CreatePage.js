...
function CreateAccount() {
  ...
  function checkvalidpassword(str) {
    ...
    console.log('nice password!: ' + password);
    ...
    if (email.length > 0) {
      ...
      if (emailRegex.test(email)) {
        ...
        let res = await axios.get(
          'https://api.sleepmedic.me:8443/account/auth/check_email/' + email,
          headers
        );
  ...
  const checkPhone = async (e) => {
    ...
    if (phone.length < 10 || phone === '') {
      ...
      setPhoneError(true);
      let res = await axios.get(
        'https://api.sleepmedic.me:8443/account/auth/check_phone/' + phone,
        headers
      );
  ...
  */
  ...
  const handleConfirmData = async (e) => {
    ...
    try {
      let res = await axios.post(
        'https://api.sleepmedic.me:8443/account/auth/create_account',
        stringify,
        { headers }
      );
      ...
      if (res) {
        ...
        try {
          let res = await axios.post(
            'https://api.sleepmedic.me:8443/account/auth/login',
            loginInfo,
            { headers }
          );
  ...
  return (
    <Box
      sx={{
        background:
          'repeating-radial-gradient(circle at -10% -10%, #717AA8 10px, #57618E, #3E4464 50% )',
    ...
    >
      ...
      >
        <img
          onMouseOut={() => setLogoSize('100%')}
          onMouseOver={() => {
            setLogoSize('120%');
          }}
          onClick={() => navigate('/')}
          src={logo}
          title={t('createAccount.logoTitle')}
          alt="Sleep-Medic Logo"
          style={{ height: '8rem', width: 'auto', scale: logoSize }}
        />
              ...
              <Grid item xs>
                ...
                >
                  ...
                  <FormHelperText sx={{ margin: 0 }}>
                    {firstError ? null : t('createAccount.firstErr')}
                  </FormHelperText>
                </FormControl>
              </Grid>
              <Grid item xs>
                ...
                >
                  ...
                  />
                  <FormHelperText sx={{ margin: 0 }}>
                    {lastError ? null : t('createAccount.lastErr')}
                  </FormHelperText>
                </FormControl>
              </Grid>
            </Grid>
            <Grid container direction={'row'}>
              <Grid item xs>
                ...
                >
                  ...
                  />
                  <FormHelperText sx={{ margin: 0 }}>
                    {birthError ? null : t('createAccount.birthdateErr')}
                  </FormHelperText>
                </FormControl>
              </Grid>
              <Grid item xs>
          ...
          <Grid container direction={'column'} spacing={1}>
            <Grid item xs>
              ...
              >
                ...
                />
                <FormHelperText sx={{ margin: 0 }}>
                  {phoneFree ? null : t('createAccount.phoneFreeErr')}
                </FormHelperText>
                <FormHelperText sx={{ margin: 0 }}>
                  {phoneError ? null : t('createAccount.phoneErr')}
                </FormHelperText>
              </FormControl>
            </Grid>
            ...
            <Grid item xs>
              ...
              >
                ...
                />
                <FormHelperText sx={{ margin: 0 }}>
                  {emailFree ? null : t('createAccount.emailFreeErr')}
                </FormHelperText>
                <FormHelperText sx={{ margin: 0 }}>
                  {emailError ? null : t('createAccount.emailErr')}
                </FormHelperText>
              </FormControl>
            </Grid>
            ...
            <Grid item xs>
              ...
              >
                ...
                />
                <FormHelperText sx={{ margin: 0 }}>
                  {confirmError ? null : t('createAccount.confirmErr')}
                </FormHelperText>
                <FormHelperText sx={{ margin: 0 }}>
                  {complexError ? null : (
